...
namespace finance
{
    public class SpravceFinanci
    {
        ...
        public void VypisPodleKategorie(string hledanaKategorie)
        {
            Console.WriteLine($"--- TRANSAKCE PRO KATEGORII: {hledanaKategorie} ---");
            bool nalezena = false;

            foreach (Transakce t in SeznamTransakci)
            {
                if (t.Kategorie.ToLower() == hledanaKategorie.ToLower())
                {
                    t.VypisDetail();
                    nalezena = true;
                }
            ...
            if (nalezena == false)
            {
                Console.WriteLine("V teto kategorii nemas zadne transakce.");
            }
            Console.WriteLine("-----------------------------------");
        }
